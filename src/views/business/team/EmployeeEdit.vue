...
function handleSave() {
  ...
  console.log('Update employee', employee.value?.id, form.value);
...
function handleCancel() {
  if (employee.value) {
    router.push(`/business/team/employees/${employee.value.id}`);
  } else {
    router.push('/business/team/employees');
  }
}
...
<template>
  <div class="page">
    <div class="page__header">
      <div>
        <h1 class="page__title">Edit {{ employee?.name ?? 'Employee' }}</h1>
        <p class="page__subtitle">Update employee details</p>
      </div>
    </div>
